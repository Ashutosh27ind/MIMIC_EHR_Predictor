...
# Ashutosh changing maximum number of thread to use :
os.environ['NUMEXPR_MAX_THREADS'] = '60'
...
def main():
    ...
    # Ashutosh performing the downcasting of the pickle csv :
    
    #print(" Printing the pickle csv df_data size BEFORE ................")
    #print(df_data.info())

    # Ashutosh Change the dtypes (int64 -> int32)
    df_data[['ID']] = df_data[['ID']].astype('int32')

    # Ashutosh Change the dtypes (float64 -> float16)
    df_data[['t']] = df_data[['t']].astype('float32')

    # Ashutosh performing the downcasting of the pickle csv :
    #print(" Printing the pickle csv df_data size AFTER ................")
    #print(df_data.info())
    

    # Ashutosh putting the restriction of number of row to read , Hardware (RAM) Issue :
    # Reading 6000 patients data from total 11754 (Testing ):

    #df_population = args.df_population = pd.read_csv(args.population_fname,nrows=6000).set_index('ID').sort_index()
    ...
    # Ashutosh performing the downcasting :
    # Ashutosh Change the dtypes (int64 -> int16)
    df_population[['mortality_LABEL']] = df_population[['mortality_LABEL']].astype('int16')
    df_population[['y_true']] = df_population[['y_true']].astype('int16')
    # convert to category :
    df_population['partition'] = df_population.partition.astype('category')
