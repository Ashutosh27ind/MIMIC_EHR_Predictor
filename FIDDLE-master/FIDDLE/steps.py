...
# Ashutosh added extra import here :
import gc 
...
from pympler import tracker, classtracker, asizeof
#import gc
...
def parse_variable_data_type(df_data, args):
    ...
    # Ashutosh added gc :
    del [df_hier_levels,df_nonhier, df_non_num_dup, d]
    gc.collect()
...
def process_time_dependent(df_data_time_series, args):
    ...
    ## Create NxLxD^ table
    df_time_series, dtypes_time_series = transform_time_series_table(df_data_time_series, args) # Check this step for memory :
    print('First print, Time elapsed: %f seconds' % (time.time() - start_time))

    ## Save intermediate files
    print("Saving files using joblib..........")
    ...
    print("Calling function map_time_series_features.........")
    X_all, X_all_feature_names, X_discretization_bins = map_time_series_features(df_time_series, dtypes_time_series, args) # Check this step also , Mostly failing here 
    
    #print(" Doing the gc collect stufff.........")
    del [df_time_series,dtypes_time_series]
    gc.collect()
    
    #print("Printing the tracker results.........")
    #print(tr.print_diff())
    ...
    print('Second Print, Time elapsed: %f seconds' % (time.time() - start_time))
...
def map_time_invariant_features(df, args):
    ...
    if args.discretize:
        ...
        # Ashutosh added gc :
        del [out,time_invariant_features]
        gc.collect()       
...
def func_encode_single_time_series(i, g, variables, variables_num_freq, T, dt, stats_functions, impute=True):
    try:
        ...
        if len(variables_non) > 0:
            ...
            # Ashutosh added gc :
            del [variables_non,df_j,df_values_j]
            gc.collect()
        ...
        if len(variables_num_freq) > 0:
            ...
            # Ashutosh added gc :
            del [df_values_i,delta_t_i,df_stats_i]
            gc.collect()
            
    except:
        print(i)
        raise Exception(i)
    #print(getsizeof(df_out))
    return i, df_out
...
def process_batch_time_series(first_arg):
    ...
    # Ashutosh added print :
    #print('time series dict size is :',getsizeof(out))
...
def transform_time_series_table(df_in, args):
    ...
    if args.parallel:
        print(" Executing 1st parallel now.. ")
        ...
        print()
        print('1st Parallel processing done', flush=True)
    ...
    # Handle IDs not in the 
    print("Creating the df_original from out dict.... ")
    ...
    # Ashutosh added gc :
    del [df_copy,out,df_original,D_timeseries]
    gc.collect()
...
def map_time_series_features(df_time_series, dtypes, args):
    ...
    #print("Inside map_time_series_features function now....")
    ...
    del [df_time_series, dtypes]
    gc.collect()
    
    # Ashutosh changing from int to int32:
    ...
    if args.discretize:
        ...
        # Ashutosh using non-parallel execution :
        print(" Executing 2nd parallel now.. ")
        ...
        if args.parallel:
            ...
            print("Printing the size of out list for tracking...")
            print(asizeof.asized(out, detail=1).format())
            
            print(" Performing the pool close now........")
        ...
        # Ashutosh using non-parallel execution :
        print(" Executing 3rd parallel now.. ")
...
#     assert X_all.dtype == int
    ...
    # Ashutosh added extra gc :
    del [feature_names_0, feature_names_1, feature_names_2]
    gc.collect()
