"""
extract_data.py
Author: Shengpu Tang, Ashutosh Kumar
...
def main():
    ...
    # Ashutosh changing size from 10000000 to 100000
    extract_events('CHARTEVENTS', 'CHARTTIME', ['VALUE'], 100000)
...
from config import config, data_path, mimic3_path
from collections import Counter
from itertools import islice

# Trying to use the Cpickle :
...
def check_nrows():
    ...
    for fname, n_rows in config['n_rows'].items():
        ...
        # Ashutosh changed chunksize from 10000000 to 100000 Ashutosh
        for df in pd.read_csv(mimic3_path + '{}.csv'.format(fname), chunksize=100000):
            actual_rows += len(df)
...
# Ashutosh added new function for date conversion :
def date_convert(date_to_convert):
    return datetime.datetime.strptime(str(date_to_convert), '%Y-%m-%d %H:%M:%S').strftime('%m/%d/%Y')
...
def extract_invariant():
    ...
    # Ashutosh added the date conversion and datetime formatting steps
    invariant['INTIME'] = invariant['INTIME'].apply(date_convert)
    invariant['DOB'] = invariant['DOB'].apply(date_convert)
    invariant['ADMITTIME'] = invariant['ADMITTIME'].apply(date_convert)

    invariant['INTIME'] = invariant['INTIME'].apply(dateutil.parser.parse)
    invariant['DOB'] = invariant['DOB'].apply(dateutil.parser.parse)
    invariant['ADMITTIME'] = invariant['ADMITTIME'].apply(dateutil.parser.parse)

    #invariant['INTIME'] = pd.to_datetime(invariant['INTIME'], format='%d%b%Y:%H:%M:%S', errors='coerce')
    #invariant['DOB'] = pd.to_datetime(invariant['DOB'], format='%d%b%Y:%H:%M:%S', errors='coerce')
    
    invariant['DOB'] = pd.to_datetime(invariant['DOB']).dt.date
    invariant['INTIME'] = pd.to_datetime(invariant['INTIME']).dt.date
    invariant['ADMITTIME'] = pd.to_datetime(invariant['ADMITTIME']).dt.date
    
    print("Printing the invariant information after date conversion applied !!! ")
    
    #invariant['INTIME'] = pd.to_pydatetime(invariant['INTIME'])
    #invariant['DOB'] = pd.to_pydatetime(invariant['DOB'])
    
    #invariant['INTIME'].to_pydatetime()
    
    #invariant['INTIME'] = invariant['INTIME'].dt.to_pydatetime()
    #invariant['DOB'] = invariant['DOB'].dt.to_pydatetime()

    # Ashutosh end above conversion

    for i in range(0,len(invariant)):
        #print(i+1,end='\r')
        invariant.loc[i,'AGE'] = ((invariant['INTIME'][i] - invariant['DOB'][i]).days)/(365)

    # v2.0 Author updated code :    
...
def _process_chunk(df, fname):
    ...
    if 'ICUSTAY_ID' not in df.columns:
        df_icu = pd.read_csv(mimic3_path + 'ICUSTAYS.csv', parse_dates=['INTIME', 'OUTTIME']).sort_values(
...
def _finalize_table(df, fname):
    ...
    if fname.lower() == 'chartevents':
        ...
        # Ashutosh created new function to chunk dictionary:
        def dict_chunks(data, SIZE):
            it = iter(data)
            for i in range(0, len(data), SIZE):
                yield {k: data[k] for k in islice(it, SIZE)}

        # Ashutosh use dictionary split:
        for item in dict_chunks(items_map, 10000):
            df['ITEMID'] = df['ITEMID'].replace(item)
            gc.collect()
    ...
    if fname.lower() == 'inputevents_mv':
        ...
        df.to_pickle(data_path + 'prep/{}.raw.p'.format('inputevents_mv'.lower()) , protocol=4)
    return df


def extract_events(fname, t_col, val_cols=[], chunksize=100000):
    """
    ...
    # Start Ashutosh : Modified for the performance improevement and out of memory error
    ...
    for df_chunk in _read_events(fname.upper(), [t_col], chunksize):
        ...
        df = df_chunk[['ICUSTAY_ID', 't', 'ITEMID'] + val_cols]

        df.to_pickle(data_path + 'prep/{}_{}.p'.format(fname.lower(), i), protocol=4)

        i += 1

    for filename in glob.glob(data_path + f'prep/{fname.lower()}*.p'):
        newDF = pd.read_pickle(filename)
        gc.collect()
        newDF = _finalize_table(newDF, filename)
        gc.collect()
        dfs.append(newDF)
        gc.collect()
    ...
    gc.collect()
    df.to_pickle(data_path + 'prep/{}.p'.format(fname.lower()), protocol=4)
    print('Done !!!')
    
    # Ashutosh end modified for the performance and RAM issue 

    # Ashutosh Remove temp pickle files from disk:
    for f in glob.glob(data_path + 'prep/*_*.p'):
        os.remove(f)

    return df

    # End Ashutosh : Modified for the performance improvement and handling the out of memory error


def extract_events_continuous(fname, t_start, t_end, val_cols=[], chunksize=100000):
    """
    Prepares the tables by calling the _process_chunk function.
...
def convert_inputevents_units():
    ...
    df = df[['ICUSTAY_ID', 't_start', 't_end', 'ITEMID', 'ORDERCATEGORYDESCRIPTION', 'AMOUNT', 'AMOUNTUOM', 'Rate',
             'RateUnit', 'Dose']]
...
def verify_output():
    ...
    count_dict = {
        ...
        'microbiologyevents': 283886,  # Updated microbiologyevents count as its failing in Assertion, Ashutosh
        'labevents': 10114036,
...
def stack_attr_columns():
    ...
    invariant_formatted = invariant_data.reset_index().melt(id_vars=['ID'], var_name='variable_name',
    ...
    with open(data_path + 'formatted/all_data.stacked.p', 'wb') as f:
        # Ashutosh adding protocol here 4 
        pickle.dump(formatted_data, f, protocol = 4)
